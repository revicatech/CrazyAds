...
import { portfolioCategoriesApi } from '../../services/adminApi'
...
export default function PortfolioCategoriesPage() {
  ...
  const load = () =>
    portfolioCategoriesApi.getAll().then(setItems).catch(() => toast.error('Failed to load')).finally(() => setLoading(false))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await portfolioCategoriesApi.update(editing, form)
        toast.success('Category updated')
      } else {
        await portfolioCategoriesApi.create(form)
  ...
  const handleDelete = async () => {
    try {
      await portfolioCategoriesApi.remove(delModal._id)
  ...
  return (
    <>
      ...
      <AdminPageHeader title="PORTFOLIO CATEGORIES">
