...
function EventGridCard({ item, index }) {
  ...
  return (
    <Link
      ...
      ref={ref}
      className={`
        group block relative overflow-hidden rounded-2xl bg-black aspect-[4/5]
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
        <h3 className="font-display text-white text-xl md:text-2xl tracking-widest mb-1 leading-tight group-hover:text-brand-red transition-colors duration-300">
          {lang === 'ar' ? item.titleAr : item.titleEn}
        </h3>
        {(item.locationEn || item.locationAr) && (
          <p className="text-white/50 text-xs tracking-wide mb-2">
...
export default function EventsListPage() {
  ...
  const categories = useMemo(() => {
    const seen = new Set()
    const cats = [{ en: t('events_filter_all', lang === 'ar' ? 'ar' : 'en'), ar: t('events_filter_all', 'ar'), _key: 'All' }]
    EVENTS.forEach((e) => {
      if (e.category?.en && !seen.has(e.category.en)) {
        seen.add(e.category.en)
        cats.push({ en: e.category.en, ar: e.category.ar || e.category.en, _key: e.category.en })
      }
    })
    return cats
  }, [EVENTS, lang])
  ...
  const filtered = activeCategory === 'All'
    ? EVENTS
    : EVENTS.filter((e) => e.category?.en === activeCategory)
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="pt-32 pb-16 px-6 max-w-[1400px] mx-auto">
        ...
        >
          <SectionHeading
            ...
            title={t('events_page_title', lang)}
            subtitle={t('events_page_subtitle', lang)}
          />

          {/* Category filter */}
          <div className="flex flex-wrap gap-2 mt-4">
            {categories.map((cat) => {
              const isActive = activeCategory === cat._key
              return (
                <button
                  key={cat._key}
      ...
      <section className="px-6 pb-24 max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((item, i) => (
            <EventGridCard key={item._id || item.id} item={item} index={i} />
          ))}
        </div>

        {filtered.length === 0 && (
          <div className="flex items-center justify-center py-24">
            <p className="text-black/40 text-lg">No events in this category yet.</p>
          </div>
        )}
      </section>
    </main>
